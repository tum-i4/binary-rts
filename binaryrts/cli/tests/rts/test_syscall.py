    ...
    def test_selection_file_modification(self):
        with temp_repo() as (remote_repo_path, remote_repo):
            with temp_clone() as (local_repo_path, local_repo):
                ...
                self.assertSetEqual(
                    {f"sample_module{TEST_ID_SEP}FooSuite{TEST_ID_SEP}FooMax"}, included_tests
                )
                self.assertSetEqual(
                    {f"sample_module{TEST_ID_SEP}FooSuite{TEST_ID_SEP}Max"}, excluded_tests
                )
                self.assertDictEqual(
                    {f"sample_module{TEST_ID_SEP}FooSuite{TEST_ID_SEP}FooMax": [file.__str__()]},
                    selection_causes,
                )
